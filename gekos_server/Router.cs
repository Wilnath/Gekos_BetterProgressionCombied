...
namespace gekos_server
{
    ...
    [Injectable]
    public class Callbacks(JsonUtil jsonUtil, HttpResponseUtil httpResponseUtil)
    {
        public ValueTask<string> HandleGetSkillPointConfig()
        {
            return new ValueTask<string>(jsonUtil.Serialize(Mod.config.skillChanges.SkillPointsSystem));
        }

        public ValueTask<string> HandleGetSkillsConfig()
        {
            return new ValueTask<string>(jsonUtil.Serialize(Mod.config.skillChanges.CustomMultipliers));
        }
    }
}
